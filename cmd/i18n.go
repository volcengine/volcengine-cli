...
var simplifiedChineseMessages = map[string]string{
	...
	"Set response output format (json|table|table-num|text|yaml|off). Default: json. table-num is table plus a row-number column. off still calls the API but skips --query evaluation.":       "设置响应输出格式（json|table|table-num|text|yaml|off）。默认：json。table-num 为表格加行号列。off 仍会发请求，但跳过 --query 求值。",
