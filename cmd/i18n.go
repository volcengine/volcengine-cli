...
var simplifiedChineseMessages = map[string]string{
	...
	"Set response output format (json|table|text|yaml|yaml-stream|off). Default: json.":                                                                                                         "设置响应输出格式（json|table|text|yaml|yaml-stream|off）。默认：json。",
	"JMESPath expression to filter/project the response before formatting.":                                                                                                                     "在格式化前用于过滤/投影响应的 JMESPath 表达式。",
