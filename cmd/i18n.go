...
var simplifiedChineseMessages = map[string]string{
	...
	"Set response output format (json|table|text|yaml|yaml-stream|off). Default: json. off still calls the API but skips --query evaluation.":                                                    "设置响应输出格式（json|table|text|yaml|yaml-stream|off）。默认：json。off 仍会发请求，但跳过 --query 求值。",
