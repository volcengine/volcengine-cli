...
func TestExpandForceAPIParamsPreservesNameCaseValueAndEmptyValue(t *testing.T) {
	c := NewContext()
	parser := NewParser([]string{
		"--force",
		"--api-param", " query =business=a=b",
		"--api-param", "Output=",
	})
	if _, err := parser.ReadArgs(c); err != nil {
		t.Fatalf("ReadArgs: %v", err)
	}
	if err := expandForceAPIParams(c); err != nil {
		t.Fatalf("expandForceAPIParams: %v", err)
	}
	if got := c.dynamicFlags.GetByName("query"); got == nil || got.GetValue() != "business=a=b" {
		t.Fatalf("expanded query = %#v, want value %q", got, "business=a=b")
	}
	if got := c.dynamicFlags.GetByName("Output"); got == nil || got.GetValue() != "" || len(got.GetValues()) != 1 {
		t.Fatalf("expanded case-sensitive empty Output = %#v", got)
	}
	if c.dynamicFlags.GetByName("output") != nil {
		t.Fatal("parameter name casing must be preserved")
	}
}

func TestExpandForceAPIParamsRejectsAmbiguity(t *testing.T) {
	tests := []struct {
		name    string
		args    []string
		wantErr string
	}{
		{name: "missing equals", args: []string{"--force", "--api-param", "query"}, wantErr: "expected Name=Value"},
		{name: "empty name", args: []string{"--force", "--api-param", "  =value"}, wantErr: "parameter name must not be empty"},
		{name: "reserved api param", args: []string{"--force", "--api-param", "api-param=value"}, wantErr: "reserved by the CLI"},
		{name: "reserved header", args: []string{"--force", "--api-param", "header=value"}, wantErr: "reserved by the CLI"},
		{name: "reserved body", args: []string{"--force", "--api-param", "body=value"}, wantErr: "reserved by the CLI"},
		{name: "duplicate explicit name", args: []string{"--force", "--api-param", "query=one", "--api-param", "query=two"}, wantErr: "cannot be specified more than once"},
		{name: "direct flag conflict", args: []string{"--force", "--Name", "direct", "--api-param", "Name=escaped"}, wantErr: "conflicts with direct --Name"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := NewContext()
			if _, err := NewParser(tt.args).ReadArgs(c); err != nil {
				t.Fatalf("ReadArgs: %v", err)
			}
			err := expandForceAPIParams(c)
...
func TestExpandForceAPIParamsConflictCheckIsCaseSensitive(t *testing.T) {
	c := NewContext()
	if _, err := NewParser([]string{
		"--force",
		"--Name", "direct",
		"--api-param", "name=escaped",
	}).ReadArgs(c); err != nil {
		t.Fatalf("ReadArgs: %v", err)
	}
	if err := expandForceAPIParams(c); err != nil {
		t.Fatalf("case-distinct names should coexist: %v", err)
	}
	if got := c.dynamicFlags.GetByName("Name"); got == nil || got.GetValue() != "direct" {
		t.Fatalf("direct Name = %#v", got)
	}
	if got := c.dynamicFlags.GetByName("name"); got == nil || got.GetValue() != "escaped" {
		t.Fatalf("escaped name = %#v", got)
	}
}

func TestUnknownNonJSONForceCallSupportsBusinessQueryAndOutput(t *testing.T) {
	previousHook := executeInvocationHook
	defer func() { executeInvocationHook = previousHook }()

	var (
		captured invocationParams
		built    invocationInput
	)
	executeInvocationHook = func(_ *Context, p invocationParams, buildInput func() (invocationInput, error)) error {
		captured = p
		var err error
	...
	err := tryExecuteGenericInvoke([]string{
		...
		"--version", "2024-01-01",
		"--endpoint", "newservice.volcengineapi.com",
		"--force",
		"--query", "Result.Id",
		"--output", "text",
		"--api-param", "query=business=a=b",
	...
	if err != nil {
		t.Fatalf("tryExecuteGenericInvoke: %v", err)
	}
	if captured.serviceName != "newservice" || captured.action != "NewAction" {
		t.Fatalf("captured invocation = %#v", captured)
	}
	if captured.contentType != "" || built.jsonBody {
		t.Fatalf("expected unknown non-JSON call, contentType=%q jsonBody=%v", captured.contentType, built.jsonBody)
	}
	input, ok := built.value.(map[string]interface{})
	if !ok {
		t.Fatalf("input type = %T, want map", built.value)
	}
	if input["query"] != "business=a=b" || input["output"] != "" {
		t.Fatalf("business query/output = %#v", input)
	}
	...
	if _, leaked := input["api-param"]; leaked {
		t.Fatalf("--api-param control leaked into request: %#v", input)
	}
}

func TestUnknownJSONForceCallExpandsAPIParamBeforeInputBuild(t *testing.T) {
	previousHook := executeInvocationHook
	defer func() { executeInvocationHook = previousHook }()

	var built invocationInput
	executeInvocationHook = func(_ *Context, _ invocationParams, buildInput func() (invocationInput, error)) error {
		var err error
		built, err = buildInput()
		return err
	}
	...
	if !built.jsonBody || built.fromBody {
		t.Fatalf("JSON force input flags: jsonBody=%v fromBody=%v", built.jsonBody, built.fromBody)
	}
	input, ok := built.value.(map[string]interface{})
	if !ok || input["query"] != "business" {
		t.Fatalf("JSON business query input = %#v", built.value)
	}
}

func TestKnownServiceUnknownActionSupportsExplicitAPIParam(t *testing.T) {
	previousHook := executeInvocationHook
	defer func() { executeInvocationHook = previousHook }()
	...
	command := &cobra.Command{Use: "sts"}
	err := runServiceCmd(command, "sts", rootSupport.GetAllAction("sts"), []string{
		"DefinitelyUnlistedAction",
		"--force",
		"--query", "Result",
		"--api-param", "query=business",
	})
	if err != nil {
		t.Fatalf("runServiceCmd: %v", err)
	}
	input, ok := built.value.(map[string]interface{})
	if !ok || input["query"] != "business" {
		t.Fatalf("known-service unknown-action input = %#v", built.value)
	}
	if got := ctx.fixedFlags.GetByName("query"); got == nil || got.GetValue() != "Result" {
		t.Fatalf("system query = %#v", got)
	}
}

func TestKnownActionForceAPIParamCannotOverrideDirectParam(t *testing.T) {
	c := NewContext()
	params := publicActionParameterNames("sts", "GetCallerIdentity")
	if _, err := NewParser([]string{
		"--force",
		"--Name", "direct",
		"--api-param", "Name=escaped",
	}, params).ReadArgs(c); err != nil {
		t.Fatalf("ReadArgs: %v", err)
	}
	err := doForceAction(c, "sts", "GetCallerIdentity")
	if err == nil || !strings.Contains(err.Error(), "conflicts with direct --Name") {
		t.Fatalf("expected direct parameter conflict, got %v", err)
	}
}

func TestKnownActionForceCallSupportsExplicitAPIParam(t *testing.T) {
	previousHook := executeInvocationHook
	defer func() { executeInvocationHook = previousHook }()
	...
	if _, err := NewParser([]string{
		"--force",
		"--output", "json",
		"--api-param", "output=business",
	}, params).ReadArgs(c); err != nil {
		t.Fatalf("ReadArgs: %v", err)
	}
	if err := doForceAction(c, "sts", "GetCallerIdentity"); err != nil {
		t.Fatalf("doForceAction: %v", err)
	}
	input, ok := built.value.(map[string]interface{})
	if !ok || input["output"] != "business" {
		t.Fatalf("known-action explicit API parameter = %#v", built.value)
	}
	if got := c.fixedFlags.GetByName("output"); got == nil || got.GetValue() != "json" {
		t.Fatalf("known-action system output = %#v", got)
	}
}
...
func TestPrintUnknownServiceHelp(t *testing.T) {
	...
	for _, flag := range []string{"--force", "--version", "--method", "--header", "--body", "--api-param", "--lang"} {
