...
Use --no-browser to skip opening the default browser.`: `登录火山引擎控制台，并在本地缓存临时 STS 凭证。

登录统一使用 OAuth 2.0 设备码授权：CLI 会输出验证地址和用户码，
并在你于任意设备完成授权前持续轮询令牌。

可使用 --no-browser 禁止自动打开默认浏览器。`,
	...
	"Deprecated: cross-device login is now the only mode":                                                         "已废弃：跨设备登录现已是唯一模式",
	"Warning: --remote is deprecated and ignored; 've login' always uses cross-device device code authorization.": "警告：--remote 已废弃且不再生效，'ve login' 统一使用跨设备的设备码授权。",
	"Do not automatically open the browser during login":                                                          "登录期间不自动打开浏览器",
...
模式：
  ...
  ve logout --all`,
	...
	"device authorization timed out; please run 've login' again":    "设备授权已超时；请重新运行 've login'",
	...
	"device code is invalid or expired; please run 've login' again": "设备码无效或已过期；请重新运行 've login'",
	"no configuration found; nothing to log out":                     "未找到配置，无需退出",
