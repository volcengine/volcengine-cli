...
- **`text` 递归摊平任意响应**：与 AWS CLI 的 text 一致，`text` 绝不输出 JSON 串。不带 `--query` 时整份响应被递归摊平成 TSV：对象的标量字段拼成一行，嵌套的对象/列表各自换行输出、并以大写的**字段路径**做前缀（如 `RESULT.INSTANCELIST\t...`）。对象列表共享一套列集合（缺失字段为 `None`），每个对象一行。更深的位置数组或对象列表投影会递归展开，嵌套空列表或空对象不产生空白行，对象列序仍遵循 `--query` 多选哈希。顶层纯标量列表会拼成一行（Tab 分隔）；需要“每条记录一行”便于接 `nl`、`grep` 时，请用 `--query` 投影成扁平列表。嵌套超过 8 层的部分与 `table` 一致，按紧凑 JSON 输出——真实响应远达不到这个深度，该上限只为异常响应兜底。
- **`text` 行标签**：第一列是该行来源节点的完整路径，因此 `awk -F'\t' '$1=="RESULT.INSTANCELIST"'` 可以精确选出记录行，记录字段从 `$2` 开始。同一个列表的所有记录共用同一个标签。嵌套值会带上所属记录的序号（从 1 开始）——`RESULT.INSTANCELIST.TAGS[2]` 就是第 2 台实例的标签——这与 `table` 对应分区的标题同名，两种格式对层级的描述完全一致。父列表只有一条记录时不加序号。注意位置化 TSV 本身没有字段名：需要稳定且自解释的列集合时，请用 `--query` 多选哈希投影。
