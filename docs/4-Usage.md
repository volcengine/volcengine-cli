...
`--profile`, `--region`, `--endpoint`, and `--lang` are the public CLI system flags. `--Param value` is an API parameter. System flags in API calls are placed after the action.
