...
`--profile`, `--region`, `--endpoint`, and `--lang` are CLI system flags and should normally precede the service. `--Param value` is an API parameter.
...
System flags use the standard double-hyphen form:
...
A system flag before the service is always parsed as a system flag. After the action, an exact case-sensitive name exposed by that action is parsed as an API parameter; otherwise it remains a system flag. Names with different casing, such as `--Region` or `--Endpoint`, are always API parameters.
...
The only current exact-name conflict is the `--lang` API parameter on `i18nopenapi VideoProjectSuppressionStart`. To set both values, use:
...
The only supported system flags are `--profile`, `--region`, `--endpoint`, and `--lang`.
