...
func jpfSortBy(arguments []interface{}) (interface{}, error) {
	intr := arguments[0].(*treeInterpreter)
	// sort_by is an expression over its input, not an in-place mutation. Work
	// on a shallow copy so repeated or concurrent searches can safely share the
	// same response value.
	source := arguments[1].([]interface{})
	arr := make([]interface{}, len(source))
	copy(arr, source)
	exp := arguments[2].(expRef)
	node := exp.ref
	if len(arr) == 0 {
