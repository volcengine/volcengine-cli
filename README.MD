...
1. 使用build.sh编译客户端
    ```shell
    # 全自动：按当前机器的 OS 和架构编译（最常用）
    sh build.sh

    # 也可显式指定 OS（架构仍自动探测），下面三选一
    # 如果是mac 
    sh build.sh darwin
    # 如果是windows
    sh build.sh windows
    ...
    # 如需交叉编译，可选传第二参数指定架构：amd64 / arm64 / 386 / arm
...
| `console-login` | 控制台登录获取临时 STS 凭证 | 通过 `ve login` 配置，写入 `login-session` |
...
mode: 凭证模式，可选值：ak, sso, console-login, ramrolearn, oidc, ecsrole；默认为 ak。其中 console-login 由 ve login 写入，不建议手动配置
access-key: 您的 AK
secret-key: 您的 SK
region: 地域，如 cn-beijing
session-token: 临时凭证的 Session Token
...
- oidc-token-file
- role-trn
- login-session（console-login 模式字段，由 ve login 写入）
- disable-ssl
- endpoint
- endpoint-resolver
- use-dual-stack
...
- `---profile`：运行时指定使用的 profile（不修改配置文件）；必须提供一个已存在的 profile 名称，缺少值或 profile 不存在时会报错
- `---region`：运行时覆盖 region
- `---endpoint`：运行时覆盖 endpoint
- `---debug`：运行时开启或关闭 CLI debug 日志，支持 `true/false`、`1/0`、`yes/no`、`on/off`
- `---debug-log-file`：将 CLI debug 日志追加写入指定文件；未指定时 debug 日志输出到 stderr，不会默认落盘
...
# 打印 CLI debug 日志到 stderr，正常 API JSON 结果仍输出到 stdout
ve ecs DescribeInstances ---debug true

# 将 CLI debug 日志追加写入指定文件
...
命令行参数优先级高于环境变量：`---debug` 覆盖 `VOLCENGINE_CLI_DEBUG`，`---debug-log-file` 覆盖 `VOLCENGINE_CLI_DEBUG_LOG_FILE`。未开启 debug 时不会输出日志，也不会创建日志文件。
